...
const SORT_OBJ = {};
const sortFuns = ["arraySort", "heapSort", "shellSort", "quickSort"];
// 导入存在的方法
sortFuns.forEach((funName) => fs.existsSync(path.resolve(__dirname, `${funName}.js`))
    && (SORT_OBJ[funName] = require(`./${funName}.js`)));
const getRandomArr = () => {
...
for (let funName in SORT_OBJ) {
    ...
    loggers.push(`${funName}: 所有的排序( ${testCounts
        ...
        }倍 ) \n${obj.extraMsg || `√√√√√√√${funName}是稳定的`}: Array.prototype.sort排序时间: ${obj.time1
        }  ${funName
...
for (let funName in SORT_OBJ) {
    ...
    for (let i = 0; i < testCounts; i++) {
        console.log(i + 1,
            '：排序正确',
            toTest(
                SORT_OBJ[funName].arr[i].sort((c, n) => n.value - c.value),
                ...args
            ),
            '\n'
        );
...
// const mid = Math.floor(loggers.length / 3);
// console.log("测试乱序的数组")
// loggers.slice(0, mid).forEach(m => console.log(m + '\n'));
// console.log("测试倒序的数组")
// loggers.slice(mid, mid + mid).forEach(m => console.log(m + '\n'));
// console.log("测试排序好的数组")
// loggers.slice(mid + mid).forEach(m => console.log(m + '\n'));